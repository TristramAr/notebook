...
看下面这个鬼二叉树，应该按照图示中的路径走才是最大路径。所以再进行递归时要不断更新路径值。
...
下面这种方法采用前n项和的方法结合哈希表存储遍历的前N个节点的和（curSum），通过再哈希表中查找curSum-Sum,来确定是否存在满足条件的路径。
...
private:
    int helper(TreeNode*root,int sum, int curSum,unordered_map<int,int>&mp)
    {
        ...
        mp[curSum]++;
        res+=helper(root->right,sum,curSum,mp)+helper(root->left,sum,curSum,mp);
        mp[curSum]--;  //回溯  
        return res;
    }
};
...
public:
    bool isSymmetric(TreeNode* root) {
        ...
        while(!q.empty())
        {
            ...
            TreeNode*t2 = q.front();
            q.pop();
            // 再迭代时这个条件不足以判断，如下图所示。而递归时的返回值是节点的左右都相等时才为真
...
![image-20200405154443953](二叉树的遍历.assets/image-20200405154443953.png)
