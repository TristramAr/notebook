...
- 当extern作用变量和函数时，``` extern int a;``` 其作用就是声明函数或者全局变量的作用范围的关键字，通过该关键字可以使得函数和变量可以在本模块或者其他模块中使用。  
...
- 修饰函数  
  static修饰函数时，使得函数只能在**包含该函数定义的文件中被调用**。对于静态函数，声明和定义需要放在同一文件夹中。  
 ...
 **堆：** new 分配的内存块（数组、类的实例等），需要delete手动释放，如果未释放完，在程序结束后系统自动回收。
 **自由储存区：** malloc分配的内存块，需要free 手动释放。
...
#### 19、C++中类和结构体的区别
- 最本质的一个区别就是默认的访问控制：**struct作为数据结构的实现体，它默认认的数据访问控制是public的**，而**class作为对象的实现体，它默认的成员变量访问控制是private的。**
...
- **析构函数是用来释放所定义的对象中使用的指针，释放分配的空间**，默认的析构函数不用显示调用，自建的析构函数要在程序末尾调用。
...
**内存溢出的原因**  
内存溢出是指程序在申请内存时，足够的内存空间供其使用。原因如下
...
基类指针可以指向派生类的对象（多态），如果删除该指针delete[]p，就会调用该指针指向的派生类的析构函数，而派生类的对象又会自动调基类的成员函数，这样就会把派生类的对象释放，如果基类的析构函数没有定义成虚函数，则编译器实现的静态绑定，在删除基类的指针，只会释放基类的对象而不会释放派生类的对象，此时会导致释放内存不完全，就会导致内存泄露的问题。
```cpp
#include <iostream>
using namespace std;
//基类的析构函数没有定义成虚函数
